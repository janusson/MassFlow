#!/usr/bin/env python3
"""
Deprecated MSP build helper.

This wrapper now delegates to ``yogimass.workflow.build_library``. Prefer using
`yogimass library build --format msp` for new workflows.
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Deprecated MSP builder. Use `yogimass library build` instead.",
    )
    parser.add_argument(
        "--input",
        nargs="+",
