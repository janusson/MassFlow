...
ref: https://blog.esciencecenter.nl/build-your-own-mass-spectrometry-analysis-pipeline-in-python-using-matchms-part-i-d96c718c68ee
ref: https://blog.esciencecenter.nl/build-a-mass-spectrometry-analysis-pipeline-in-python-using-matchms-part-ii-spec2vec-8aa639571018
ref: https://blog.esciencecenter.nl/build-a-mass-spectrometry-analysis-pipeline-in-python-using-matchms-part-iii-molecular-91891248ee34
ref: https://matchms.readthedocs.io/en/latest/index.html
"""
...
logger.info("Starting {__name__} module.")


# * Settings
def load_settings(libraries_path=r"./database"):
    """
    Loads Yogimass build settings.
    ...
    libraries_path = r"./database"  # yogimass library directory
    # ? Below is an example of an MGF file in need of processing.
    # ? Names out of order, missing metadata, etc.
    reference_msp_lib = os.path.join(libraries_path, "eric_lib.msp")
...
# * List currently available libraries (MGF and MSP)
...
def metadata_processing(spectrum):
    ...
    # spectrum = derive_inchi_from_smiles(spectrum) #TODO Requires RDKit
    # spectrum = derive_smiles_from_inchi(spectrum) #TODO Requires RDKit
    # spectrum = derive_inchikey_from_inchi(spectrum) #TODO Requires RDKit
...
def clean_mgf_library(mgf_path):
    ...
    # # TODO Can this operation be vectorised with numpy?
...
def clean_msp_library(msp_path):
    ...
    Returns:
        list: List of cleaned and processed spectra. #? NOTE returns a generator?
    """
    from matchms.importing import load_from_msp

    print(f"Cleaning {msp_path} library spectra...")
    # library_array = np.array(
    #     load_from_mgf(mgf_path)).tolist()  # TODO Can this operation be vectorised with numpy?
...
def get_spectrum_inchi(spectrum):
    """
    Get spectra INCHI key from MGF spectrum
    """
    return spectrum.get("inchikey")


# ? For testing, generate an example cleaned library:
#! Resource intensive operation!
#! See new script for handling library cleaning: build_library_from_msp.py
# default_library_spectra = clean_mgf_library(reference_msp_lib)
# save_spectra_to_mgf(default_library_spectra, r'./out/default_library_spectra.mgf')
...
def top10_cosine_matches(reference_library, query_spectra, spectrum_number=1):
    ...
    return top10_smiles  # TODO: return top10_smiles??


# def show_smiles(matches):
#     smiles_list = [x[0].get('smiles' for x in matches)]
#     return smiles_list
...
def modified_cosine_scores(reference_library, check_spectra):
    ...
    Args:
        reference_library (match_ms): [description]
        check_spectra ([type]): [description]
...
def main():
    ...
    libraries_path, reference_msp_lib = load_settings(libraries_path=r"./database/")
    msp_list = list_msp_libraries(r"../database/msp databases")
    cleaned_library = clean_msp_library(os.path.abspath(msp_list[10]))
    return print(f"{__name__} complete")
