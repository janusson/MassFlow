...
def _ms1_prefilter(
    ...
    ref_missing = np.array(
        [r is None or np.isnan(float(r)) for r in ref_mzs_raw], dtype=bool
    )
    query_missing = np.array(
        [q is None or np.isnan(float(q)) for q in query_mzs_raw], dtype=bool
    )
