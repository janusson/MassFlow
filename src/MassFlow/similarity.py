...
def _ms1_prefilter(
    ...
    ref_missing = np.array([r is None or np.isnan(r) for r in ref_mzs_raw], dtype=bool)
    query_missing = np.array([q is None or np.isnan(q) for q in query_mzs_raw], dtype=bool)

    ref_mzs = np.array([float(r) if r is not None and not np.isnan(r) else 0.0 for r in ref_mzs_raw])
    query_mzs = np.array([float(q) if q is not None and not np.isnan(q) else 0.0 for q in query_mzs_raw])
