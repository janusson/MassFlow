...
def metadata_processing(
    ...
    Args:
        ...
        config: Optional ProcessingConfig to inject instrument metadata.
    ...
    if config:
        if config.instrument:
            spectrum.set("instrument", config.instrument)
        if config.mode:
            spectrum.set("ionmode", config.mode)
...
def peak_processing(spectrum: Spectrum, config: ProcessingConfig) -> Optional[Spectrum]:
    ...
    # 2. Filter Peak Count
    spectrum = require_minimum_number_of_peaks(spectrum, n_required=config.min_peaks)
    if spectrum is None:
