...
| `processing.py` | ~500 | **Required** | `matchms`-based metadata/peak pipeline + the strict 5 ppm physical-integrity gate (`physical_integrity_reason`), which consumes the `MassFlow.models` contracts on spectra with declared structural claims | Gate rejections are reported via the optional `rejection_reporter` and the quarantine logger; see CAPABILITY_MATRIX D-8 |
...
| `workflow.py` | ~1300 | **Required** | Orchestration, per-file failure model (`FileExecutionResult`), provenance, TDC block, worker-owned library backends, library-lineage sidecar payload (`_build_library_provenance`) | Lineage payload attaches to every per-file result and is written into the YAML sidecar (`library:` section); `LibrarySpec` mechanism unchanged |
| `library.py` | ~530 | **Required** | Library normalization (`prepare_library`), worker-openable `LibrarySpec`, `RawFileLibraryStore`, public `processing_fingerprint` | Fresh raw-library builds record a `library_builds` row (best-effort, sqlite/hybrid stores only) |
...
| `database.py` | ~2600 | **Required** | `SpectralDatabase` (SQLite/hybrid store), spectra + library-provenance schema (`store_meta`, `library_builds`, `PRAGMA user_version` >= 1), migrations, lineage queries (`record_library_build`, `get_library_builds`, `get_latest_library_build`, `get_store_created_at`, `get_schema_version`). Contains legacy-schema migration helpers (`migrate_legacy_peaks_database`, `_decode_legacy_peaks_payload`, `LegacyDatabaseSchemaError`) | Provenance schema is additive raw SQL (no ORM/migration framework); recording is explicit (never on plain `add_spectra`), so scientific payload bytes are unaffected; legacy helpers are **deprecated tooling** for pre-array-schema DBs |
...
| `models.py`, `cheminformatics.py` | 386+727 | **Required** | 5 ppm precursor validation, adduct registry, isotopic envelopes | Enforced as a processing gate on spectra with declared structural claims in the classical `annotate`/`db build` paths (D-8 executed; see CAPABILITY_MATRIX C-11/D-8) plus the streaming gate; `processing.py` hosts the gate (`physical_integrity_reason`) |
